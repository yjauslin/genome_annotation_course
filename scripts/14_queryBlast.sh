...
blastp -query $WORKDIR/results/final/assembly.all.maker.proteins.renamed.filtered.longest.fasta -db $COURSEDIR/data/uniprot/uniprot_viridiplantae_reviewed.fa -num_threads 10 -outfmt 6 -evalue 1e-5 -max_target_seqs 10 -out $OUTDIR/blastp_output_uniprot # Now sort the blast output to keep only the best hit per query sequence 
