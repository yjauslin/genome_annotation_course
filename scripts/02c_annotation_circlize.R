...
superfamilies <- superfamilies[!superfamilies %in% c("helitron", "long_terminal_repeat", "repeat_fragment", "repeat_region", "target_site_duplication")]
...
lgd <- Legend(
    title = "Superfamily", at = c("genes", superfamilies),
    legend_gp = gpar(fill = colors[1:length(c("genes", superfamilies))])
)
